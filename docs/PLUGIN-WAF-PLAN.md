...
> **วัดแล้ว (T504, 2026-09-20): 97.8% ดิบ ๆ และ 100% หลังชั้นแปลงเล็ก ๆ**
> ไม่มีกฎข้อไหนเลยที่ต้องใช้ backreference หรือ lookaround เพราะ CRS
> **ห้ามไว้เป็นนโยบายที่เขียนไว้ใน `CONTRIBUTING.md`** เพื่อให้เข้ากับ engine
> ตระกูล RE2 — ซึ่ง `regex` ของ Rust อยู่ในตระกูลนั้น
> รายละเอียดและผลที่ตามมาทั้งหมดอยู่ใน
> [`docs/decisions/0001-waf-engine.md`](decisions/0001-waf-engine.md)
...
> **สองข้อแรกวัดแล้วใน T504 และผลไม่ตรงกับที่เขียนไว้ตอนวางแผน**
> — `RegexSet` ช้ากว่า naive **18 เท่า** (ถอดออก) และ prefilter ช่วยแค่ **2 เท่า**
> ไม่ใช่ O(rules)→O(1) ส่วน naive เองก็อยู่ที่ 82 µs/request ซึ่งอยู่ใน budget แล้ว
> ดู [decision record](decisions/0001-waf-engine.md) หัวข้อ "สิ่งที่วัดแล้วขัดกับแผน"
...
1. **Literal prefilter ตัวเดียวสำหรับทั้ง ruleset** — ดึง literal substring ออกจาก regex ของทุกกฎ
   ...
   ~~นี่คือตัวที่เปลี่ยนจาก O(rules) เป็น O(1)~~ **วัดแล้วได้ 2× (ข้าม regex ได้ 34.8%)**
   เพราะมีแค่ 110 จาก 312 pattern ที่ดึง literal ใช้กรองได้
2. ~~**`RegexSet` ต่อ target**~~ — **ถอดออก** วัดแล้วช้ากว่า naive 18 เท่า
   และ `RegexSet` ของ CRS ทั้งชุดยังไม่ผ่าน size limit 10 MB ที่เป็นค่าเริ่มต้นของ crate
